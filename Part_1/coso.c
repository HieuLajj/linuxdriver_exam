...
int docso_h10(int soh10){
	...
	x=*kernel_buffer;
	while(x!='\0'){
		pr_info("gia tri cua x lan luot la: %d",x);
		soh10= soh10*10+(x-48);
		kernel_buffer++;
		pr_info("gia tri cua kernel_buffer la: %c",kernel_buffer);
		x=*kernel_buffer;
	}
...
void chuyen10_2(int soh10){
 ...
 static int __init lab5_init(void){
	pr_info("heloo \n");
	alloc_chrdev_region(&lab5.dev_num,0,1,"lab5_laivanhieu");
	lab5.dev_class=class_create(THIS_MODULE, "Lab5");
	lab5.dev=device_create(lab5.dev_class, NULL, lab5.dev_num,NULL,"lab5");
